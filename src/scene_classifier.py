...
class SceneContextMLP(nn.Module):
    ...
    def filter(self, proposed_action, cls_token, threshold=0.4):
        ...
        self.eval()
        with torch.no_grad():
            # CAUSE E fix: ensure cls is 1D and on CPU
            cls_token = cls_token.squeeze().cpu()
            scores = self.forward(cls_token).squeeze()  # (3,)

        score_dict = {a: round(scores[ACTION_IDX[a]].item(), 3)
                      for a in ACTION_IDX}
        chosen_score = scores[ACTION_IDX[proposed_action]].item()

        print(f"[Paper3] proposed={proposed_action} "
              f"score={chosen_score:.3f} all={score_dict}")

        if chosen_score < threshold:
            best_idx = scores.argmax().item()
            override = IDX_ACTION[best_idx]
            print(f"[Paper3] OVERRIDE -> {override}")
            return override

        return proposed_action

    def get_scores(self, cls_token):
        """Return action scores as a plain Python list [move_fast, stop_wait, navigate].

        Used by the dashboard to render the Paper 3 bar chart.
        """
        self.eval()
        with torch.no_grad():
            cls_token = cls_token.squeeze().cpu()
            scores = self.forward(cls_token).squeeze()  # (3,)
        return [scores[i].item() for i in range(3)]
