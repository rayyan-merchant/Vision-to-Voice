...
class SaliencyEngine:
    ...
    # ── AttnLRP (Fallback to SmoothGrad due to lxt incompatibility) ──────
    def attnlrp_map(self, input_tensor):
        """Generate attribution map using high-quality SmoothGrad.
        (lxt is incompatible with current transformers version, using SmoothGrad).
        """
        return self.smoothgrad_map(input_tensor)

    # ── SmoothGrad ───────────────────────────────────────────────
    def smoothgrad_map(self, input_tensor, target_class=None, n_samples=2):
        """Generate SmoothGrad attribution map (Paper 5).

        Args:
            input_tensor: (1, 3, 224, 224)
            target_class: output neuron index to attribute. If None, targets the argmax.
            n_samples: number of noisy copies (default 50)

        Returns:
            numpy array (224, 224) in [0, 1]
        """
        # Fix A: Ensure gradients flow even if created in torch.no_grad()
        x = input_tensor.detach().clone().requires_grad_(True)
        
        with torch.enable_grad():
            if target_class is None:
                out = self.dino_model(x)
                target_class = int(out[0].argmax().item())

            attrs = self.smoothgrad.attribute(
                x,
                nt_type="smoothgrad",
                nt_samples=n_samples,      # spec: 50 noisy copies
                stdevs=0.15,        # spec: noise std dev
                target=target_class,
            )
        ...
        # Fix C: Percentile clipping to expand dynamic range
        p99 = np.percentile(smap, 99)
        smap = np.clip(smap, 0, p99)
        
        # Quality Improvement: Gaussian blur to make spatial clusters coherent
        smap = cv2.GaussianBlur(smap, (7, 7), sigmaX=1.5)
